...
export async function createUpload(
  ...
  try {
    ...
    await prisma.rawUpload.update({
      ...
      data: {
        status: "ERROR",
        errorMessage: "Falha ao enfileirar processamento",
      },
    });
    throw err;
  }
...
export async function getColumnMapping(tenantId: string, id: string) {
  ...
  if (!readyStatuses.includes(upload.status)) {
    throw new ValidationError(
      `Upload ainda não está pronto para mapeamento. Status atual: ${upload.status}`,
    );
  }
...
export async function confirmMapping(
  ...
  await prisma.rawUpload.update({
    where: { id },
    data: { mapping, branchId, status: "PENDING" },
  });
