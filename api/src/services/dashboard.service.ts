...
export async function getSalesTimeline({
  ...
  const timeline: Record<
    number,
    { currentYear: number | null; previousYear: number | null }
  > = {};
  for (let m = 1; m <= 12; m++) {
...
export async function getBranchesRanking({
  tenantId,
  from,
  to,
}: DashboardFilters) {
      ...
      return {
        ...
        goalAchievement:
          goalAchievement === null
            ? null
            : Number.parseFloat(goalAchievement.toFixed(2)),
      };
    })
...
export async function getProjection({ tenantId, branchId }: DashboardFilters) {
  ...
  if (branchId) {
    ...
    monthlyGoal = result._sum?.monthlyGoal
      ? Number(result._sum.monthlyGoal)
      : null;
  }
